...
@implementation NolanoSdkContext
...
+ (instancetype)initWithParams:(NSDictionary *)params {
    ...
    NolanoSdkContext *context = [[NolanoSdkContext alloc] init];
    if (context->llama == nullptr) {
        context->llama = new nolanosdk::llama_rn_context();
    }
...
- (NSArray *)tokenProbsToDict:(std::vector<nolanosdk::completion_token_output>)probs {
    ...
    for (const auto &prob : probs)
    {
        ...
        {
            std::string tokStr = nolanosdk::tokens_to_output_formatted_string(self->llama->ctx, p.tok);
            [probsForToken addObject:@{
                @"tok_str": [NSString stringWithUTF8String:tokStr.c_str()],
                @"prob": [NSNumber numberWithDouble:p.prob]
            }];
        }
        std::string tokStr = nolanosdk::tokens_to_output_formatted_string(self->llama->ctx, prob.tok);
        [out addObject:@{
    ...
    onToken:(void (^)(NSDictionary * tokenResult))onToken
{
    ...
    while (llama->has_next_token) {
        const nolanosdk::completion_token_output token_with_probs = self->llama->doCompletion();
        ...
        size_t stop_pos =
            self->llama->findStoppingStrings(str_test, token_text.size(), nolanosdk::STOP_FULL);
        if (stop_pos != std::string::npos) {
            ...
            stop_pos = self->llama->findStoppingStrings(str_test, token_text.size(),
                nolanosdk::STOP_PARTIAL);
        }

        const std::string to_send = self->llama->generated_text.substr(pos, stop_pos);
        sent_count += to_send.size();

        std::vector<nolanosdk::completion_token_output> probs_output = {};
        ...
        if (self->llama->params.n_probs > 0) {
            ...
            if (probs_pos < probs_stop_pos) {
                probs_output = std::vector<nolanosdk::completion_token_output>(self->llama->generated_token_probs.begin() + probs_pos, self->llama->generated_token_probs.begin() + probs_stop_pos);
            }
